...
def folders_walk(data_folder):
    'added by pren1'
    data_dict = {}
    'get first level folders'
    first_level_folders = [f.path for f in os.scandir(data_folder) if f.is_dir()]
    'for each folder, get second level folders'
    for first in first_level_folders:
        ...
        for second in second_level_folders:
            second += '/'
            test = glob.glob(os.path.join(second, '**', hp.form.input), recursive=True)
            this_level_res.extend(test)

        data_dict[first.split('/')[-1]] = this_level_res
    return data_dict
...
if __name__ == '__main__':
    ...
    test_dict = folders_walk(input_path + 'test/')
    test_spk = [x for x in test_dict if len(x) >= 2]

    # pdb.set_trace()
    # train_folders = [f.path for f in os.scandir(input_path) if f.is_dir()]
    #
    #
